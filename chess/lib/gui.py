...
from chess.lib import availableMoves
...
def drawPieces(win, board, flip, player, flags):
    side = player
    opside = 0
    if int(side) == 0:
        opside = 1
    ...
    for x, y, ptype in board[side]:
        ...
        for i in availableMoves(side, board, [x, y, ptype], flags):
            x = 470 - i[0] * 50 if flip else i[0] * 50 + 20
            y = 470 - i[1] * 50 if flip else i[1] * 50 + 20
            for p in board[opside]:
                if i[0] == p[0] and i[1] == p[1]:
                    win.blit(CHESS.PIECES[opside][p[2]], (p[0] * 50, p[1] * 50))
